...
The dedicated `@sulaf/cli` acts as a seamless wrapper, letting you add components with zero extra configuration.
...
```json
{
  "aliases": { ... },
  "registries": {
    "@sulaf": "https://sulaf.netlify.app/r/components/{name}.json"
  }
}
```
...
```bash
npx shadcn-vue@latest add https://sulaf.netlify.app/r/components/autocomplete.json
```
